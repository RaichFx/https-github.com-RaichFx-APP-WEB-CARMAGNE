...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onBack, currentUser, theme, setTheme }) => {
          ...
          filteredTools.map(tool => (
            <div key={tool.id} className="bg-[var(--panel-bg)] p-5 rounded-[2rem] border border-[var(--panel-border)] flex flex-col justify-between group hover:border-amber-500/50 transition-colors shadow-xl">
              ...
              <div className="space-y-3 pt-4 border-t border-[var(--panel-border)]">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center text-[var(--text-muted)]"><Users size={14} /></div>
                  <div>
                ...
                {tool.siteName && (
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center text-[var(--text-muted)]"><MapPin size={14} /></div>
                    <div>
  ...
  const handleGenerateAndSendPayslip = async () => {
    ...
    const baseSalary = Number(payslipForm.baseSalary) || 0;
    const extraHours = Number(payslipForm.extraHours) || 0;
    const extraHoursPay = Number(payslipForm.extraHoursPay) || 0;
    const deductions = Number(payslipForm.deductions) || 0;
    const totalPay = baseSalary + (extraHours * extraHoursPay) - deductions;
  ...
  const renderReports = () => (
    <div className="space-y-6 animate-fadeIn pb-32">
      <div>
        <h2 className="text-2xl font-bebas text-neon uppercase">Partes Semanales (IA Gemini)</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {weeklyReports.length > 0 ? (
          weeklyReports.map(report => (
            <div key={report.id} className="mirror-panel p-6 flex flex-col justify-between hover:border-neon transition-all">
              <div className="space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-bebas text-lg text-white uppercase">{report.workerName}</h3>
                ...
                <div className="relative aspect-video rounded-xl bg-[var(--island-bg)] overflow-hidden group">
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all">
                    <button onClick={() => { setSelectedReport(report); setShowRejectionInput(false); }} className="bg-neon text-black p-3 rounded-full font-bold text-xs flex items-center gap-1">
                ...
                {report.isAiParsed && (
                  <div className="space-y-2 p-3 bg-[var(--island-bg)] rounded-xl border border-[var(--panel-border)]">
                    <p className="text-[9px] text-neon font-bold uppercase">Lectura de IA (Gemini):</p>
                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                      ...
                      <div className="col-span-2">
                        ...
                        <span className="font-bold text-neon">{report.extractedTotal || '-'}</span>
                      </div>
                    </div>
                  </div>
                )}
        ...
        <div className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-xl flex items-center justify-center p-6 animate-fadeIn">
          <div className="bg-[var(--modal-bg)] w-full max-w-lg rounded-[2.5rem] border border-[var(--modal-border)] p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className="text-xl font-bebas text-neon uppercase">Detalles del Parte Semanal</h3>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 bg-[var(--input-bg)] p-5 rounded-2xl border border-[var(--panel-border)]">
                ...
                <div>
                  ...
                  <span className="text-sm font-black text-neon">{selectedReport.extractedHours || 0} horas</span>
                </div>
                <div>
                  <span className="text-[9px] text-[var(--text-muted)] font-bold block uppercase tracking-wider">Total Calculado (IA)</span>
                  <span className="text-sm font-black text-neon">{selectedReport.extractedTotal || '-'}</span>
                </div>
  ...
  const renderPayslips = () => {
    ...
    return (
      <div className="space-y-8 animate-fadeIn pb-32">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bebas text-neon uppercase">Gestión de Nóminas</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 mirror-panel p-6 space-y-6">
            <h3 className="text-lg font-bebas text-neon uppercase">Enviar Nueva Nómina</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-[var(--island-bg)] rounded-2xl border border-[var(--panel-border)] space-y-1">
                ...
                <span className="text-2xl font-black text-neon">{calculatedTotal.toFixed(2)}€</span>
              </div>

              <button onClick={handleGenerateAndSendPayslip} className="w-full bg-neon text-black py-4 rounded-xl font-bold uppercase text-xs shadow-lg shadow-neon/15 hover:shadow-neon/30 active:scale-95 transition-all">
          ...
          <div className="lg:col-span-2 space-y-4">
            <h3 className="text-lg font-bebas text-neon uppercase">Nóminas Enviadas</h3>
                  ...
                  {payslips.length > 0 ? (
                    payslips.map(ps => (
                      <tr key={ps.id} className="hover:bg-[var(--btn-glass-bg)]/50 transition">
                        ...
                        <td className="p-4 font-black text-neon">{ps.totalPay.toFixed(2)}€</td>
                        <td className="p-4">
                          <span className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded-full ${
                            ps.status === 'SIGNED' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-blue-500/10 text-blue-400 border border-blue-500/20'
                          }`}>
                        ...
                        <td className="p-4 text-right">
                          {ps.pdfBase64 && (
                            <a href={ps.pdfBase64} download={`Nomina_${ps.workerName.replace(/\s+/g, '_')}_${ps.monthStr}.pdf`} className="inline-flex p-2 bg-neon/10 text-neon rounded-lg border border-neon/20 hover:bg-neon hover:text-black transition">
                              <Download size={14} />
                            </a>
                          )}
                        </td>
    ...
    <div className="flex h-screen bg-[var(--bg-color)] text-[var(--text-main)] overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
          ...
          {activeTab === 'workers' && (
            <div className="space-y-4 animate-fadeIn pb-32">
              <div className="flex justify-between items-center"><h2 className="text-xl font-black text-[var(--text-main)] uppercase">Personal</h2><button className="bg-blue-600 p-3 rounded-xl text-white"><UserPlus size={20}/></button></div>
              <div className="grid gap-2">{filteredWorkers.map(w=>(<div key={w.id} className="bg-[var(--panel-bg)] p-4 rounded-2xl border border-[var(--panel-border)] flex justify-between items-center"><div><p className="font-black text-[var(--text-main)] text-sm uppercase">{w.name}</p><p className="text-[9px] text-[var(--text-muted)] font-bold uppercase tracking-widest">{w.dni || 'S/DNI'}</p></div><div className="flex gap-1"><button onClick={()=>setReportModal({...reportModal, isOpen:true, worker:w})} className="p-2 text-emerald-500"><FileText size={20}/></button><button onClick={()=>StorageService.deleteWorker(w.id)} className="p-2 text-rose-500"><Trash2 size={20}/></button></div></div>))}</div>
            </div>
          )}
          ...
          {activeTab === 'admins' && isSuperAdmin && (
             <div className="space-y-6 animate-fadeIn pb-32"><div className="flex justify-between items-center"><h2 className="text-xl font-black text-[var(--text-main)] uppercase">Cuentas Admin</h2><button onClick={() => setIsAdminModalOpen(true)} className="bg-indigo-600 p-3 rounded-xl text-white"><UserPlus size={20} /></button></div><div className="grid gap-3">{admins.map(admin => (<div key={admin.id} className="bg-[var(--panel-bg)] p-4 rounded-3xl border border-[var(--panel-border)] flex justify-between items-center"><div className="flex items-center gap-4"><div className="w-10 h-10 bg-slate-800 rounded-2xl flex items-center justify-center text-[var(--text-muted)] border border-[var(--panel-border)]"><KeyRound size={20} /></div><div><h3 className="text-sm font-black text-[var(--text-main)]">{admin.username}</h3><p className="text-[10px] text-[var(--text-muted)] font-bold uppercase tracking-widest">Gestor</p></div></div><button onClick={() => StorageService.deleteAdmin(admin.id)} className="p-2 text-rose-500"><Trash2 size={20} /></button></div>))}</div></div>
          )}
