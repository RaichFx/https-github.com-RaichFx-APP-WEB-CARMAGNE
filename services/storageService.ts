    ...
    try {
      ...
      if (changedWorkers.length > 0) {
        await Promise.all(changedWorkers.map(async w => {
          try {
            const workerPayload = safeClone(w) as Record<string, any>;

            // Los datos públicos/locales llevan las credenciales vacías. Nunca deben
            // sobrescribir el hash real guardado en Firestore durante una edición de perfil.
            if (!String(workerPayload.pinHash || '').startsWith('pbkdf2_sha256
          } catch (err) {
  ...
  deleteCertificateDoc: async (certId: string) => {
    ...
    try {
      ...
      console.error("Error deleting certificate doc:", e);
              ...
              delete workerPayload.pinHash;
            }
            if (!String(workerPayload.pin || '').trim()) {
              delete workerPayload.pin;
            }

            await setDoc(doc(db, "workers", w.id), workerPayload, { merge: true });
          } catch (err) {
            console.error(`Error saving worker ${w.id} / ${w.name}:`, err);
            throw err;
          }
